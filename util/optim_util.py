...
def get_optimizer(parameters, args):
    if args.optimizer == 'sgd':
        ...
        raise ValueError('error, util/optim_util/get_optimizer')
    return optimizer
...
def step_scheduler(lr_scheduler, metrics=None, epoch=None, global_step=None, best_metric='val_loss'):
    """Step a LR scheduler."""
    if isinstance(lr_scheduler, optim.lr_scheduler.ReduceLROnPlateau):
        if best_metric in metrics:
            lr_scheduler.step(metrics[best_metric], epoch=epoch)
