...
class PENetClassifier(nn.Module):
    ...
    def __init__(self, model_depth=50, cardinality=32, num_channels=3, num_classes=1, init_method=None, **kwargs):
...
class PENet_Temp(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.img_encoder = PENetClassifier()

        #w 加载预训练参数
        module = self.img_encoder
        tar_path = '/mntcephfs/lab_data/wangcm/wangzhipeng/ckpt/penet.pth.tar'
        load_pretrained_model(module, tar_path)

    def forward(self, x):
        out, fea = self.img_encoder(x)
        return out, fea